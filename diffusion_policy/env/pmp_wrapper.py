...
class ManiSkillEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # Image.fromarray(img_xy.astype(np.uint8)).save("/home/icrlab/imitation/improved_dp/train_logs/step_img_xy.png")
        # Image.fromarray(img_xz.astype(np.uint8)).save("/home/icrlab/imitation/improved_dp/train_logs/step_img_xz.png")
        # Image.fromarray(img_yz.astype(np.uint8)).save("/home/icrlab/imitation/improved_dp/train_logs/step_img_yz.png")
        
        # _,_,_, pmp_xy, pmp_xz, pmp_yz = project_to_tripleplane(
        #     pointcloud=point_cloud[..., :3],
        #     rgb=point_cloud[...,3:],
        #     voxel_size=0.005,
        #     img_size=(84, 84),
        ...
        left_rgb = cv2.resize(sensor_data['left_camera']['rgb'], (168, 168), interpolation=cv2.INTER_AREA)
        right_rgb = cv2.resize(sensor_data['right_camera']['rgb'], (168, 168), interpolation=cv2.INTER_AREA)
        resized_image = cv2.resize(rgb, (168, 168), interpolation=cv2.INTER_AREA)
