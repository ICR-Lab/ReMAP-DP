...
class TransformerForDiffusion(ModuleAttrMixin):
    ...
    def forward(self,
        ...
        output: (B,T,input_dim) or ((B,T,input_dim), list of activations) if return_act=True
        """
        # 1. time
        timesteps = timestep
        ...
        # Store intermediate activations if needed (like original paper)
        act = []
        ...
        if self.encoder_only:
            ...
            # Manual loop through encoder layers to collect activations (like original paper)
            if return_act:
                for layer in self.encoder.layers:
                    x = layer(x, src_mask=self.mask)
                    act.append(x)  # Collect activation from each layer
            else:
            ...
            # Manual loop through decoder layers to collect activations (like original paper)
            if return_act:
                for layer in self.decoder.layers:
                    x = layer(x, memory, tgt_mask=self.mask, memory_mask=self.memory_mask)
                    act.append(x)  # Collect activation from each layer
            else:
